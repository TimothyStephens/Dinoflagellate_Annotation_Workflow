...
for SEQ in ${GENOME_NAME}.softmasked_dir/*;
        do
	echo "${SNAP}/snap ../${GENOME_NAME}.snap.hmm ${SEQ} -lcmask -quiet 1> ${SEQ}.snap 2> ${SEQ}.snap.stderr; ${SCRIPTS}/SNAP_output_to_gff3.pl ${SEQ}.snap ${SEQ} 1> ${SEQ}.snap.gff3 2>${SEQ}.snap.gff3.stderr" >> snap.commands
        done
