...
run_cmd "${PASA}/Launch_PASA_pipeline.pl -c alignAssembly.config -C -R -g ${GENOME_NAME} --MAX_INTRON_LENGTH ${MAX_INTRON_LENGTH} --ALIGNERS blat --CPU ${NCPUS} -T -t transcripts.fasta.clean -u transcripts.fasta --TDN TrinityDN.accs > pasa.log 2>&1"
