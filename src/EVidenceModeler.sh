...
run_cmd "${EVIDENCEMODELER}/EvmUtils/partition_EVM_inputs.pl --genome ${GENOME_NAME} --gene_predictions abinitio_gene_predictions.gff3 --segmentSize 50000000 --overlapSize 10000 --partition_listing partitions_list.out"

run_cmd "${EVIDENCEMODELER}/EvmUtils/write_EVM_commands.pl --genome ${GENOME_NAME} --gene_predictions abinitio_gene_predictions.gff3 --weights `pwd`/evm_weights.txt --output_file_name evm.out --partitions partitions_list.out > commands.list"
